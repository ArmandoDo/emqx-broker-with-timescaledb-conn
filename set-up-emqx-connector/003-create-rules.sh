...
create_units_status_rule() {
    curl -X POST http://localhost:18083/api/v5/rules \
        ...
        -d '{
            ...
            "sql": "SELECT payload.timestamp AS timestamp, topic AS topic, qos AS qos, flags.retain AS retain, flags.dup AS dup, payload.dispenser_id AS dispenser_id, payload.session_id AS session_id, payload.energy_kwh AS energy_kwh, payload.voltage_v AS voltage_v, payload.current_a AS current_a, payload.temperature_c AS temperature_c, payload.status AS status, payload.station_id AS station_id FROM \"sst/units/+/status\"",
            "enable": true,
